...
def test_api_balance_corrected():
    """使用正确的API路径测试用户 19825012076 的余额"""
    
    print("=" * 60)
    print("使用正确的API路径测试用户 19825012076 的余额")
    print("=" * 60)
        ...
        if result.returncode == 0 and result.stdout:
            try:
                ...
                if response_data.get('success'):
                    ...
                    login_result = subprocess.run(curl_login, shell=True, capture_output=True, text=True, timeout=10)
                    ...
                    if login_result.returncode == 0 and login_result.stdout:
                        try:
                            ...
                            if login_data.get('success'):
                                ...
                                # 3. 获取余额
                                print(f"\n3. 使用Token获取余额...")
                                curl_balance = f'curl -X GET "http://localhost:8080/api/wallet/balance" -H "Authorization: {token}" -H "Content-Type: application/json"'
                                print(f"命令: {curl_balance}")
                                
                                balance_result = subprocess.run(curl_balance, shell=True, capture_output=True, text=True, timeout=10)
...
if __name__ == "__main__":
    print("开始使用正确的API路径测试用户 19825012076 的余额...")
    
    # 测试API
    success = test_api_balance_corrected()
    
    if not success:
        print("\n尝试使用curl命令测试...")
        test_with_curl_corrected()
